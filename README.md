...
一个基于 MCP (Model Context Protocol) 和本地 Ollama 大模型的简单用户数据库查询程序。支持自然语言查询用户信息，通过 LLM 理解用户意图并调用相应工具获取数据。
...
可用工具:
  ...
  • get_user_relationships - 查询用户的所有关系
  • get_relationship_between_users - 查询两人之间的关系
  • get_spouse - 查询配偶信息
  • get_relatives_by_relation - 按关系类型查询亲戚
  • get_children - 查询子女信息
  • get_parents - 查询父母信息
...
### 示例 1: 多轮关系查询 - Alice 的女儿有朋友吗？

**输入：**
```
你: Alice 的女儿有朋友吗？如果有的话，他叫什么名字？
```
...
🤖 [来自 LLM 的原始响应文本 - 第 1 轮]:
{"tool": "query_users", "arguments": {"name": "Alice"}}
======================================================================
[16:28:23] LLM 响应完成 (耗时: 27.16秒)
...
🤖 [来自 LLM 的原始响应文本 - 第 2 轮]:
{"tool": "get_children", "arguments": {"user_id": 1}}
======================================================================
[16:28:50] LLM 响应完成 (耗时: 26.18秒)
...
🤖 [来自 LLM 的原始响应文本 - 第 3 轮]:
{"tool": "get_user_relationships", "arguments": {"user_id": 13}}
======================================================================
[16:29:10] LLM 响应完成 (耗时: 19.17秒)

[工具调用] get_user_relationships
[工具参数] {'user_id': 13}
[工具结果] {
  "user": {
    "id": 13,
  ...
  "relationships": [
    ...
    {
      "related_user_id": 2,
      "related_user_name": "Bob",
      "relation": "女父"
    },
    {
      "related_user_id": 3,
      "related_user_name": "Charlie",
      "relation": "妹兄"
    },
    {
      "related_user_id": 7,
      "related_user_name": "Grace",
      "relation": "朋友"
    }
  ]
}
...
🤖 [来自 LLM 的原始响应文本 - 第 4 轮]:
Alice 的女儿 Mia 有朋友，她的朋友名字是 **Grace**。
======================================================================
[16:29:27] LLM 响应完成 (耗时: 16.25秒)

[16:29:27] LLM 给出最终答案
[总耗时] 91.07秒
[工具调用次数] 3

🤖 AI 发给用户的回答: Alice 的女儿 Mia 有朋友，她的朋友名字是 **Grace**。
```

**这个例子展示了多轮工具调用的过程：**
1. 第 1 轮：查询 Alice 用户信息
2. 第 2 轮：获取 Alice 的子女列表，确定女儿是 Mia（ID=13）
3. 第 3 轮：查询 Mia 的所有关系，发现她有个朋友 Grace
4. 第 4 轮：LLM 拥有足够信息，给出最终答案
...
你: 名字中包含"David"的用户的邮箱是什么

你: Charlie 的父母分别有哪些子女
